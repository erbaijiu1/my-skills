...
生成或修改后端业务方法、前端组件和脚手架中的多步骤逻辑时：

- 同一业务目的的语句紧密放在一起；不同步骤、子步骤之间空一行，循环内部同样适用。例如参数准备、业务处理、响应组装按职责分组。
- 空行放在步骤注释之前，注释紧贴所属代码；该步骤需要的 Map、集合等初始化也放在注释下面，不留在上一步末尾。
- 不按固定行数或逐句加空行，不强制给简单方法添加编号注释；遵循目标语言与项目格式化规范，不改变缩进和字符串内容。
- 交付前检查注释、准备代码和处理逻辑是否属于同一组，步骤边界是否清晰；只调整本次生成或修改的代码，不扩展为无关文件的格式重构。
