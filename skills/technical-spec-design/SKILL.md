...
## Pseudocode readability: proximity principle

Apply this when a specification contains multistep pseudocode; it does not change the prohibition on runnable implementation code.

- Group operations with the same business purpose together and separate distinct stages or substages with one blank line, including inside loops.
- Keep a stage label or comment directly adjacent to the operations it describes. Put the separating blank line before it, and include stage-specific preparation beneath it.
- Use spacing to communicate responsibility boundaries, not a fixed line count or one blank line per operation. Do not force numbered labels onto trivial examples.
- Before delivery, check that each pseudocode group has a coherent purpose and that stage transitions are visible. Apply this to the specification being produced or edited, without rewriting unrelated documents.
